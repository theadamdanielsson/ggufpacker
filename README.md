...
A publisher ships 15-25 quant variants per model. Every one of them is a deterministic function of a single F16 source: run `llama-quantize` with a given type (and, for k-quants, an imatrix) and you get that variant back. So there is no reason to store 16 GB of near-duplicate weights. ggufpack stores the F16 source once, plus a tiny recipe per file and a small zstd "correction delta", and regenerates each quant on demand.

## Quickstart

```
pip install git+https://github.com/theadamdanielsson/ggufpack
```

(PyPI package coming shortly.)

You also need a `llama-quantize` binary (from a llama.cpp build) and the F16 source present in the directory you pack. ggufpack invokes `llama-quantize` to prove and later reproduce each file; it does not ship one.

Pack a directory:

```
ggufpack pack ./Llama-3.2-1B-Instruct-GGUF -o llama-1b.ggufpack --llama-quantize /path/to/llama-quantize
```

Inspect what was stored:

```
ggufpack stats llama-1b.ggufpack
```

Regenerate a single quant (by type or by filename). Output is always sha256-verified against the original; ggufpack refuses to emit a file on mismatch:

```
ggufpack unpack llama-1b.ggufpack Q4_K_M -o Q4_K_M.gguf
```

Re-verify the whole store end to end:

```
ggufpack verify llama-1b.ggufpack
```
...
- **blob** — the whole file, zstd-compressed. This is the automatic fallback for anything that cannot be expressed as a recipe (the F16 source itself, and files ggufpack does not know how to regenerate). It is never lossy.
...
Every plan is executed and hash-verified **at pack time**, before it is recorded. ggufpack actually runs the recipe, compares the result to the original by sha256, and only then writes the plan into the store. If a plan does not reproduce, ggufpack downgrades it (NEAR, or blob) until it has something that verifies. The pack you get is one whose every file has already been proven to reconstruct bit-exact.
...
Stated up front, because they change what ggufpack is good for today:
...
**Is this lossy?**
No. Every emitted file is sha256-verified against the original, and ggufpack refuses to emit on a mismatch. Plans are also proven at pack time. If a byte is wrong, you get an error, not a file.

**Why not just zstd the directory?**
Because byte compressors get about 1.15x on quantized weights — they are already high-entropy. ggufpack gets 8.7x because it does not compress the weights, it regenerates them from the source.
